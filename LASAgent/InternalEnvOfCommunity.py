...
class InternalEnvOfCommunity(object):
    ...
    def _partition_reward(self, observation_partition, 
        ...
        for agent_name in observation_partition.keys():
            obs_interest_ir = np.zeros(agent_community_partition_config[agent_name]['observation_space'].shape)
            for i, name in enumerate(agent_community_partition_config[agent_name]['observation_space_name']):
                # Get IR sensor info
                if 'ir_node' in name:
                    obs_interest_ir[i] = observation_partition[agent_name][i]
            reward_partition[agent_name] = np.sum(obs_interest_ir)
        return reward_partition
    
    def _partition_reward_light(self, observation_partition, 
                          agent_community_partition_config):
        """
        Partition red light reward based on observation_partition and 
        agent_community_partition_config. (This is only used in simulation.)
        ...
        for agent_name in observation_partition.keys():
            ...
            for i, name in enumerate(agent_community_partition_config[agent_name]['observation_space_name']):
                ...
                if 'ir_node' in name:
                    obs_interest_ir[i] = observation_partition[agent_name][i]
                if 'light' in name:
                    obs_interest_light.append(observation_partition[agent_name][i])
            #
            if len(obs_interest_light) / 3 != 0:
                raise Exception('Light number is not ')
            for i in range():
                obs_interest_light
            reward_partition[agent_name] = np.sum(obs_interest_ir)
        return reward_partition
